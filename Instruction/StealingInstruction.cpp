...
{
    this->boardModel->printMessage(QString("STEALING FROM %1")
                                  .arg(this->what));
    this->boardModel->printMessage(QString("Try stealing from %1.")
                                  .arg(this->what));
    this->boardModel->printMessage("As long as you draw event cards with gold nuggets on them, you gain gold and may keep on stealing.");
    this->boardModel->printMessage(QString("But once you've stolen gold, but do not draw gold nuggets when trying to steal more, you are caught, "
...
Instruction *StealingInstruction::triggerDone()
{
    ...
    if(this->step == 0)
    {
        ...
        if(goldNuggets > 0)
        {
            this->boardModel->printMessage(QString("You stole %1 gold nuggets on your first try.")
                                          .arg(goldNuggets));
        }
        else
        {
            this->boardModel->printMessage("You draw no gold nuggets.");
            this->boardModel->printMessage("The stealing attempt failed and you've NOT been caught.");
            this->boardModel->printMessage(" ");
            this->boardModel->printMessage("Press Done to Continue...");
            this->boardModel->printMessage(" ");
            this->done = true;
    ...
    if(this->step == 2)
    {
        ...
        if(goldNuggets > 0)
        {
            this->boardModel->printMessage(QString("You steal %1 gold nuggets for a total of %2 gold nuggets stolen so far.")
        ...
        else
        {
            ...
            this->boardModel->printMessage(QString("Your attempt failed and you've been caught.\nYou fled with half your stash (rounded up) for a total of %1 gold gained.")
                                          .arg(this->stash));
            this->boardModel->printMessage(QString("But, the %1 are attacking now.")
                                          .arg(this->what));
            this->step = 3;
        }
    ...
    if(this->step == 3)
    {
        ...
        this->boardModel->printMessage(QString("The active region in which the %1 are attacking is %2.")
                                      .arg(this->what)
    ...
    if(this->step == 4)
    {
        ...
        this->boardModel->printMessage(QString("Adding up the GREEN SQUARE and BLUE HEXAGON number results in an attacking force of %1.").arg(attackForce));
        this->boardModel->printMessage(" ");
...
void StealingInstruction::decisionMade(QAbstractButton *button)
{
    ...
    switch(buttonRole)
    {
        case QMessageBox::Yes:
            this->step = 2;
            this->boardModel->printMessage("You try to steal some more gold.\nPress Done to continue.");
            break;
        default:
            this->done = true;
            this->step = -1;
            this->boardModel->gainGold(this->stash);
            this->boardModel->printMessage(QString("You decided to stop stealing and collect the gold you stole so far.\n"
                                          "You receive %1 Gold.\n"
                                          "Press Done to Continue...")
                                          .arg(this->stash));
            break;
